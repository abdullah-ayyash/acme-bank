...
app.post("/auth", function (request, response) {
  ...
  if (username && password) {
    db.get(
      `SELECT * FROM users WHERE username = $username AND password = $password`,{$password: request.body.password,$username: request.body.username},
...
app.post("/transfer", function (request, response) {
  if (request.session.loggedin) {
    ...
    if (account_to && amount) {
      if (balance > amount) {
        db.get(
          `UPDATE users SET balance = balance + $amount WHERE account_no = $account_to`,{$amount: amount,$account_to: account_to},
          function (error, results) {
            console.log(error);
            console.log(results);
          }
        ...
        db.get(
          `UPDATE users SET balance = balance - $amount WHERE account_no = $account_from`,{$amount: amount,$account_from: account_from},
          function (error, results) {
...
app.post("/download", function (request, response) {
  if (request.session.loggedin) {
    ...
    const fileName = path.normalize(root_directory);
    try {
...
app.post("/public_forum",check('comment').notEmpty().isLength({ min: 5, max: 200 }).blacklist('<>'), function (request, response) {
  if (request.session.loggedin) {
    ...
    if (comment) {
      db.all(
        `INSERT INTO public_forum (username,message) VALUES ($username,$comment)`,{$username: username,$comment: comment},
        (err, rows) => {
          console.log(err);
        }
      );
...
app.get("/public_ledger", function (request, response) {
  if (request.session.loggedin) {
    ...
    if (id) {
      db.all(
        `SELECT * FROM public_ledger WHERE from_account = $id`,{$id: request.query.id},
        (err, rows) => {
